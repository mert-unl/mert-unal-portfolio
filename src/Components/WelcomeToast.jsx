...
const {setLang,langData} = useContext(AppContext)
...
useEffect(() => {
  const toastActive = langFlag === true || questionFlag === false;
...
function onAcceptClick(){
  ...
  function trLangHandle(){
      setLang("tr") 
      setLangFlag(false)
  }

  function enLangHandle(){
      setLang("en") 
      setLangFlag(false)
  }
 ...
 const langButton = "font-bold text-4xl duration-600 px-8 transition-all hover:scale-120 rounded-md p-4" 
  ...
  return (
        <div className={`${questionFlag ?"display:none hidden " :"fixed inset-0 flex flex-col items-center justify-center z-100 bg-black " }`}>
          ...
          <div className={`${langFlag ? 'text-white flex flex-row  mt-20  gap-12 text-2xl':"display:none hidden "}`}> 
              <button  className={`${langButton} hover:bg-red-600 bg-red-700`} onClick={trLangHandle}>TR</button>
              <button className={`${langButton} hover:bg-blue-600  bg-blue-700`} onClick={enLangHandle}>EN</button>
          </div>
